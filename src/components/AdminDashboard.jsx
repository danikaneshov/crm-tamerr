...
const AdminDashboard = () => {
 ...
 {/* График ЗП */}
 <div className="bg-white backdrop-blur-xl border border-white p-8 rounded-[40px] smooth-shadow">
 ...
 <div className="bg-gradient-to-br from-emerald-700 to-green-700 p-8 rounded-[32px] shadow-lg shadow-emerald-900/50 text-white relative overflow-hidden md:col-span-2">
 ...
 <tbody className="divide-y divide-slate-50">{employees.map(emp => (<tr key={emp.id} className={emp.isArchived ? 'opacity-50 bg-slate-50 ' : ''}><td className="p-6 font-bold text-slate-900 ">{emp.name}</td><td className="p-6 font-mono text-slate-500">{emp.pin}</td><td className="p-6">{emp.isArchived ? <span className="text-xs bg-slate-200 text-slate-500 px-3 py-1 rounded-full font-bold">Архив</span> : <span className="text-xs bg-emerald-900/30 text-green-600 px-3 py-1 rounded-full font-bold">Активен</span>}</td><td className="p-6 text-right">{emp.isArchived ? (<button onClick={() => updateDoc(doc(db, 'employees', emp.id), { isArchived: false })} className="text-xs font-bold text-emerald-500 hover:text-green-700 px-3 py-1.5 bg-green-50 rounded-xl hover:bg-emerald-900/30 transition-colors">Восстановить</button>) : (<button onClick={() => { if (window.confirm(`Деактивировать ${emp.name}? Все данные по ЗП сохранятся.`)) updateDoc(doc(db, 'employees', emp.id), { isArchived: true }); }} className="text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={18}/></button>)}</td></tr>))}</tbody></table>
 </div>
 </div>
 )}
 </div>
 ...
 <Card variant="elevated" className="p-8 card-hover-effect">
 <div className="flex items-center gap-4 mb-4"><div className="w-12 h-12 bg-gradient-to-br from-slate-700 to-slate-900 rounded-2xl flex items-center justify-center text-white text-xl"></div><div><p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Мундштуки</p><h3 className="text-3xl font-black text-slate-900 ">{formatMoney(Math.round(mouthpieceStock))} шт</h3></div></div>
 <div className="bg-slate-50 p-4 rounded-2xl space-y-2 text-sm border border-slate-100">
