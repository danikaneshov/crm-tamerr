...
const InventoryTab = () => {
 ...
 <h3 className="text-3xl font-black text-white">≈ {Math.max(0, Math.floor(Math.min(coalStock / invStandards.coalPerBowl, tobaccoStock / (invStandards.tobaccoPerBowl || 1))))} чаш</h3>
 ...
 <div className="mt-3 px-4 py-2 bg-slate-100 rounded-xl border border-slate-200 text-center"><span className="text-slate-800 font-black text-sm">≈ {Math.max(0, Math.floor(tobaccoStock / (invStandards.tobaccoPerBowl || 1)))} чаш</span></div>
 </Card>

 <Card variant="elevated" className="p-8 card-hover-effect">
 <div className="flex items-center gap-4 mb-4"><div className="w-12 h-12 bg-gradient-to-br from-slate-700 to-slate-900 rounded-2xl flex items-center justify-center text-white text-xl shadow-inner shadow-white/20"></div><div><p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Мундштуки</p><h3 className="text-3xl font-black text-slate-900 ">{formatMoney(Math.round(mouthpieceStock))} шт</h3></div></div>
 <div className="bg-slate-50 p-4 rounded-2xl space-y-2 text-sm border border-slate-100">
 ...
 <div className="mt-3 px-4 py-2 bg-slate-100 rounded-xl border border-slate-200 text-center"><span className="text-slate-800 font-black text-sm">≈ {Math.max(0, Math.floor(mouthpieceStock / (invStandards.mouthpiecePerBowl || 1)))} чаш</span></div>
 </Card>
 </div>
 </div>
 )}
