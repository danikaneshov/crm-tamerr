...
const OperationsPanel = () => {
 ...
 <button onClick={() => setOperationType('in')} className={`flex-1 sm:flex-none px-6 py-2 rounded-xl font-bold text-sm transition-all ${operationType === 'in' ? 'bg-white text-green-600 shadow-sm border border-slate-100' : 'text-slate-500 hover:text-slate-800 '}`}>Приход</button>
 <button onClick={() => setOperationType('writeoff')} className={`flex-1 sm:flex-none px-6 py-2 rounded-xl font-bold text-sm transition-all ${operationType === 'writeoff' ? 'bg-white text-orange-500 shadow-sm border border-slate-100' : 'text-slate-500 hover:text-slate-800 '}`}>Списание</button>
 </div>
 </div>
 ...
 key={t.id} 
 onClick={() => addToCart(t)}
 className="bg-white backdrop-blur-md p-4 rounded-2xl border-none smooth-shadow hover:border-blue-500 hover:shadow-md transition-all text-left flex flex-col gap-2 group active:scale-95"
 >
 ...
 {invCart.length > 0 && (
 <div className="bg-white backdrop-blur-xl rounded-[32px] border-none smooth-shadow p-6">
 <h2 className="text-lg font-black text-slate-800 mb-6">Корзина прихода</h2>
 ...
 <p className="font-black text-slate-800 ">{formatMoney(item.pricePerUnit * item.quantity)} ₸</p>
 </div>
 <button onClick={() => removeFromCart(item.id)} className="text-slate-300 hover:text-red-500 p-2 rounded-xl hover:bg-red-50">Удалить</button>
 </div>
 </div>
 ))}
 </div>
 ...
 <span className="text-emerald-500">+{formatMoney(m.amount)} {m.item === 'coal' || m.item === 'mouthpiece' ? 'шт' : 'г'}</span>
 </p>
 ...
 <button onClick={() => deleteDoc(doc(db, 'inventory_movements', m.id))} className="text-slate-300 hover:text-red-500">
 <Trash2 size={16}/>
 </button>
 </div>
 </div>
 ...
 <div className="bg-white backdrop-blur-xl p-8 rounded-[40px] border-none smooth-shadow max-w-xl">
 ...
 {/* История списаний */}
 <div className="bg-white rounded-[32px] border-none smooth-shadow overflow-hidden max-w-xl">
